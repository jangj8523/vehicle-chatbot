...
class Encoder(Module):
    ...
    def regularizer(self, encoder_input):
        ...
        Recall the KL divergence term for KL(vMF(k), vMF(0)) in d dims is (in mathematica format)
        k *( (BesselI[d/2+1,k]+d/2*BesselI(d/2,k)/k) / BesselI[d/2,k] - d / 2k) 
        + d*log(k)/2 - log(BesselI[d/2,k]) - log(Gamma[d/2+1]) - d*log(2)/2 
        
        we incur 4X this number, since we have a vMF for the insert, delete, insert_exact, and delete_exact.
        
        The edit norm KL divergence is?? 

        Args:
            encoder_input (EncoderInput): input sequence (source/target pair).

        Returns:
            regval (float): regularization value calibrated to be on the same scale as the loss for the batch (need to take mean).
        """
        k = self.lamb_reg
        ds = self.edit_dim/2
        #TODO(Tatsu): Correctly derive the KL regularizer penalty
        regval = 0  # self.edit_encoder.noise_scaler
        return self.lamb_reg * regval
